...
public class DruidClientTest {
    ...
    public void testAutoClosing() {
        Assertions.assertThat(wasClientClosed)
                .withFailMessage("Client closing indicator is not initialized properly")
                .isFalse();
        try (TestDruidClient client = new TestDruidClient()) {
            client.connect();
        }
        Assertions.assertThat(wasClientClosed)
                .withFailMessage("Close method has not been called")
                .isTrue();
    }
